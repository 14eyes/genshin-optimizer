...
const data = {
    ...
    skill: {
        paw: [41.92, 45.06, 48.21, 52.4, 55.54, 58.69, 62.88, 67.07, 71.26, 75.46, 79.65, 83.84, 89.08, 94.32, 99.56],
        shieldHPFlat: [693, 762, 837, 918, 1005, 1097, 1195, 1299, 1409, 1524, 1646, 1773, 1905, 2044, 2188],
    ...
    burst: {
        ...
        cdmg: [52.64, 56.59, 60.54, 65.8, 69.75, 73.7, 78.96, 84.22, 89.49, 94.75, 100.02, 105.28, 111.86, 118.44, 125.02],
        healHP: [5.34, 5.74, 6.14, 6.67, 7.07, 7.47, 8, 8.54, 9.07, 9.6, 10.14, 10.67, 11.34, 12.01, 12.67],
        healHPFlat: [513, 565, 620, 680, 744, 813, 885, 962, 1044, 1129, 1219, 1313, 1411, 1514, 1621]
    }
}
...
const formula = {
    ...
    charged: {
        aimedShot: (tlvl, stats) => basicDMGFormula(data.charged.aimedShot[tlvl], stats, "charged"),
    ...
    skill: {
        paw: (tlvl, stats) => basicDMGFormula(data.skill.paw[tlvl] + (stats.constellation >= 2 ? 15 : 0), stats, "skill"),
        shield: (tlvl, stats) => {
            const percent = (data.skill.shieldHP[tlvl] + (stats.constellation >= 2 ? 15 : 0)) / 100
            const flat = data.skill.shieldHPFlat[tlvl]
            return [s => percent * s.finalHP + flat, ["finalHP"]]
        },
        shieldhold: (tlvl, stats) => {
            const percent = (data.skill.shieldHP[tlvl] + (stats.constellation >= 2 ? 15 : 0)) / 100
            const flat = data.skill.shieldHPFlat[tlvl]
            return [s => (percent * s.finalHP + flat) * 1.75, ["finalHP"]]
        }
    },
    burst: {
        dmg: (tlvl, stats) => basicDMGFormula(data.burst.dmg[tlvl], stats, "burst"),
        cdmg: (tlvl, stats) => basicDMGFormula(data.burst.cdmg[tlvl], stats, "burst"),
        regen: (tlvl, stats) => {
